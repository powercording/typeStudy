...
// 위의 인터페이스는 다음과 같은 형식으로 구체화 할수 있습니다 (잘못된 예시)
...
// 타입정보를 명시적으로 정의하였으므로 다른 곳으로 갑싱 전달되어도 타입정보가 유지됩니다.
...
// 어떤 객체 배열에서 한 필드의 값만 추출하는 함수를 작성한다고 가정하겠습니다.
...
const releasDate = keyOfFluck(albums, "releasDate"); //질문: 왜 string|Date 로 추론될까요? // 다른 키값에 대한 타입을 추론해서 일까요.
